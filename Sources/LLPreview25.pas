...
uses
  ...
  cmbtLL25x, ListLabel25, LLReport_Types, l25CommonInterfaces;
...
type
 ...
 {$ifend}
  ...
  private
    ...
    FParentComponent: TLlPreviewControl25;
  ...
  public
    constructor create(AParent: TLlPreviewControl25);
  published
...
published
  ...
  TLlPreviewControl25 = class(TCustomControl)
  private
  ...
  public
    ...
    procedure Attach(FParentComponent: TListLabel25; nFlags: Cardinal);
    procedure Detach;
...
begin
  ...
  case nMsg of
    LS_VIEWERCONTROL_QUEST_BTNSTATE:
      begin
          TLlPreviewControl25(lUserParam).QuestButtonState(lParam,lResult);
      end;

    LS_VIEWERCONTROL_NTFY_PAGELOADED:
      begin
          if Assigned(TLlPreviewControl25(lUserParam).OnPageChanged) then
                  TLlPreviewControl25(lUserParam).OnPageChanged(TLlPreviewControl25(lUserParam),lParam);
      end;

    LS_VIEWERCONTROL_NTFY_BTNPRESSED:
      begin
          If Assigned(TLlPreviewControl25(lUserParam).OnButtonClicked) then
          begin
            TLlPreviewControl25(lUserParam).OnButtonClicked(TLlPreviewControl25(lUserParam),lParam, IsHandled);
...
{ TLlPreviewPage }
  ...
  var
    ...
    length: integer;
  begin
    length := cmbtls25x.LlStgsysGetPageOptionString(FHandle, FPageIndex,
      OptionIndex, nil, 0);
    if length>0 then
    begin
      GetMem(Buffer, length * sizeof(TChar));
      Buffer^ := #0;
      cmbtls25x.LlStgsysGetPageOptionString(FHandle, FPageIndex, OptionIndex, Buffer, length);
  ...
  begin
    Result := cmbtls25x.LlStgsysGetPageOptionValue(FHandle, FPageIndex, OptionIndex);
  end;
  ...
  begin
    cmbtls25x.LlStgsysSetPageOptionString(FHandle, FPageIndex,
...
constructor TLlPreviewButtons.create(AParent: TLlPreviewControl25);
begin
  inherited create;
...
constructor TLlPreviewPageStyle.create(AParent: TLlPreviewControl25);
begin
  inherited create;
  fParentComponent:= AParent;
end;
...
constructor TLlPreviewSelectionFrame.create(AParent: TLlPreviewControl25);
begin
  inherited create;
  fParentComponent:= AParent;
end;
...
constructor TLlPreviewControl25.Create(AOwner: TComponent);
begin
...
procedure TLlPreviewControl25.CreateParams(var Params: TCreateParams);
begin
    inherited CreateParams(Params);
    Params.ExStyle:=Params.ExStyle or WS_EX_CLIENTEDGE;
end;

procedure TLlPreviewControl25.CreateWindowHandle(
...
procedure TLlPreviewControl25.CreateWnd;
begin
    inherited CreateWnd;
...
procedure TLlPreviewControl25.DestroyWindowHandle;
begin
...
procedure TLlPreviewControl25.Detach;
var
...
destructor TLlPreviewControl25.Destroy;
begin
    ToolbarButtons.Free;
    PageStyle.Free;
    SelectionFrame.Free;
...
function TLlPreviewControl25.GetActualButtonState(ButtonID: integer):integer;
begin
    if ShowToolbar then
...
function TLlPreviewControl25.GetOptionString(Option: TString): TString;
var Size: integer;
    Buffer: PTChar;
begin
...
procedure TLlPreviewControl25.SetPreviewPageStyle(const Value: TLlPreviewPageStyle);
begin
  FPreviewPageStyle:= Value;
end;

procedure TLlPreviewControl25.SetPreviewSelectionFrame(const Value: TLlPreviewSelectionFrame);
begin
  fPreviewSelectionFrame:= Value;
end;

function TLlPreviewControl25.GetCurrentPage: integer;
begin
     result:=SendMessage(handle, LS_VIEWERCONTROL_GET_PAGE, 0, 0);
end;

function TLlPreviewControl25.GetPageCount: integer;
begin
     result:=SendMessage(handle, LS_VIEWERCONTROL_GET_PAGECOUNT, 0, 0);
end;

procedure TLlPreviewControl25.GotoFirst;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_PAGE, 0, 0);
end;

procedure TLlPreviewControl25.GotoLast;
var
    Pages: integer;
begin
...
procedure TLlPreviewControl25.GotoNext;
var
    CurrentPage: integer;
begin
...
procedure TLlPreviewControl25.SaveAs;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SAVE_TO_FILE, 0, 0);
end;


procedure TLlPreviewControl25.GotoPrevious;
...
function TLlPreviewControl25.PrintAllPages(ShowPrintOptions: boolean): integer;
var Value: integer;
begin
...
function TLlPreviewControl25.PrintCurrentPage(ShowPrintOptions: boolean): integer;
var Value: integer;
begin
    if ShowPrintOptions then Value:=1 else Value:=0;
...
function TLlPreviewControl25.PWideToString(pw: PWideChar): AnsiString;
var
  buffer : PAnsiChar;
  iLen : integer;
begin
...
procedure TLlPreviewControl25.QuestButtonState(ButtonID: integer;
...
procedure TLlPreviewControl25.RefreshOptions;
begin
...
procedure TLlPreviewControl25.RefreshToolbar;
begin
    if not FInitializing then
        SendMessage(handle, LS_VIEWERCONTROL_NTFY_TOOLBARUPDATE, 0, 0);
end;

procedure TLlPreviewControl25.SetBackgroundColor(const Value: TColor);
begin
...
procedure TLlPreviewControl25.SetCloseMode(const Value: TCloseMode);
begin
  FCloseMode := Value;
end;

procedure TLlPreviewControl25.SetFileName(const Value: TString);
begin
  if not (csDesigning in ComponentState) then
  begin
    if not FInitializing then
...
function TLlPreviewControl25.GetSaveAsFilePath: TString;
var Buffer: PTChar;
begin
...
procedure TLlPreviewControl25.SetShowToolbar(const Value: boolean);
...
procedure TLlPreviewControl25.SetShowUnprintableArea(const Value: boolean);
var IntValue: integer;
begin
...
procedure TLlPreviewControl25.SetToolbarButtons(
  const Value: TLlPreviewButtons);
begin
  FToolbarButtons := Value;
end;

procedure TLlPreviewControl25.SetZoom(Percentage: integer);
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_ZOOM, Percentage, 0);
end;


procedure TLlPreviewControl25.ZoomReset;
begin
    SendMessage(handle,LS_VIEWERCONTROL_RESET_ZOOM, 0, 0);
end;

procedure TLlPreviewControl25.ZoomRevert;
begin
    SendMessage(handle,LS_VIEWERCONTROL_POP_ZOOM, 0, 0);
end;

procedure TLlPreviewControl25.SelectSlideshowTransition;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SELECT_THEATERXFORM , 0, 0);
end;

procedure TLlPreviewControl25.ZoomTimes2;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_ZOOM_TWICE, 0, 0);
end;

procedure TLlPreviewControl25.Attach(FParentComponent: TListLabel25;
  nFlags: Cardinal);
begin
...
procedure TLlPreviewControl25.SearchFirst(SearchString: TString; CaseSensitive: Boolean);
...
procedure TLlPreviewControl25.SearchNext;
begin
  SendMessage(handle, $400 + 29 (*LS_VIEWERCONTROL_SEARCH*), 1 (*LS_VCITEM_SEARCH_NEXT*), 0);
end;

function TLlPreviewControl25.CanClose: boolean;
begin
...
procedure TLlPreviewControl25.SetOnButtonClicked(
  const Value: TButtonClickedEvent);
begin
  FOnButtonClicked := Value;
end;

procedure TLlPreviewControl25.SetShowThumbnails(const Value: boolean);
var IntValue: integer;
begin
