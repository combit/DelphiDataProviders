...
uses
  ...
  cmbtLL26x, ListLabel26, LLReport_Types, l26CommonInterfaces;
...
type
 ...
 {$ifend}
  ...
  private
    ...
    FParentComponent: TLlPreviewControl26;
  ...
  public
    constructor create(AParent: TLlPreviewControl26);
  published
...
published
  ...
  TLlPreviewControl26 = class(TCustomControl)
  private
  ...
  public
    ...
    procedure Attach(FParentComponent: TListLabel26; nFlags: Cardinal);
    procedure Detach;
...
begin
  ...
  case nMsg of
    LS_VIEWERCONTROL_QUEST_BTNSTATE:
      begin
          TLlPreviewControl26(lUserParam).QuestButtonState(lParam,lResult);
      end;

    LS_VIEWERCONTROL_NTFY_PAGELOADED:
      begin
          if Assigned(TLlPreviewControl26(lUserParam).OnPageChanged) then
                  TLlPreviewControl26(lUserParam).OnPageChanged(TLlPreviewControl26(lUserParam),lParam);
      end;

    LS_VIEWERCONTROL_NTFY_BTNPRESSED:
      begin
          If Assigned(TLlPreviewControl26(lUserParam).OnButtonClicked) then
          begin
            TLlPreviewControl26(lUserParam).OnButtonClicked(TLlPreviewControl26(lUserParam),lParam, IsHandled);
...
{ TLlPreviewPage }
  ...
  var
    ...
    length: integer;
  begin
    length := cmbtls26x.LlStgsysGetPageOptionString(FHandle, FPageIndex,
      OptionIndex, nil, 0);
    if length>0 then
    begin
      GetMem(Buffer, length * sizeof(TChar));
      Buffer^ := #0;
      cmbtls26x.LlStgsysGetPageOptionString(FHandle, FPageIndex, OptionIndex, Buffer, length);
  ...
  begin
    Result := cmbtls26x.LlStgsysGetPageOptionValue(FHandle, FPageIndex, OptionIndex);
  end;
  ...
  begin
    cmbtls26x.LlStgsysSetPageOptionString(FHandle, FPageIndex,
...
constructor TLlPreviewButtons.create(AParent: TLlPreviewControl26);
begin
  inherited create;
...
constructor TLlPreviewPageStyle.create(AParent: TLlPreviewControl26);
begin
  inherited create;
  fParentComponent:= AParent;
end;
...
constructor TLlPreviewSelectionFrame.create(AParent: TLlPreviewControl26);
begin
  inherited create;
  fParentComponent:= AParent;
end;
...
constructor TLlPreviewControl26.Create(AOwner: TComponent);
begin
...
procedure TLlPreviewControl26.CreateParams(var Params: TCreateParams);
begin
    inherited CreateParams(Params);
    Params.ExStyle:=Params.ExStyle or WS_EX_CLIENTEDGE;
end;

procedure TLlPreviewControl26.CreateWindowHandle(
...
procedure TLlPreviewControl26.CreateWnd;
begin
    inherited CreateWnd;
...
procedure TLlPreviewControl26.DestroyWindowHandle;
begin
...
procedure TLlPreviewControl26.Detach;
var
...
destructor TLlPreviewControl26.Destroy;
begin
    ToolbarButtons.Free;
    PageStyle.Free;
    SelectionFrame.Free;
...
function TLlPreviewControl26.GetActualButtonState(ButtonID: integer):integer;
begin
    if ShowToolbar then
...
function TLlPreviewControl26.GetOptionString(Option: TString): TString;
var Size: integer;
    Buffer: PTChar;
begin
...
procedure TLlPreviewControl26.SetPreviewPageStyle(const Value: TLlPreviewPageStyle);
begin
  FPreviewPageStyle:= Value;
end;

procedure TLlPreviewControl26.SetPreviewSelectionFrame(const Value: TLlPreviewSelectionFrame);
begin
  fPreviewSelectionFrame:= Value;
end;

function TLlPreviewControl26.GetCurrentPage: integer;
begin
     result:=SendMessage(handle, LS_VIEWERCONTROL_GET_PAGE, 0, 0);
end;

function TLlPreviewControl26.GetPageCount: integer;
begin
     result:=SendMessage(handle, LS_VIEWERCONTROL_GET_PAGECOUNT, 0, 0);
end;

procedure TLlPreviewControl26.GotoFirst;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_PAGE, 0, 0);
end;

procedure TLlPreviewControl26.GotoLast;
var
    Pages: integer;
begin
...
procedure TLlPreviewControl26.GotoNext;
var
    CurrentPage: integer;
begin
...
procedure TLlPreviewControl26.SaveAs;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SAVE_TO_FILE, 0, 0);
end;


procedure TLlPreviewControl26.GotoPrevious;
...
function TLlPreviewControl26.PrintAllPages(ShowPrintOptions: boolean): integer;
var Value: integer;
begin
...
function TLlPreviewControl26.PrintCurrentPage(ShowPrintOptions: boolean): integer;
var Value: integer;
begin
    if ShowPrintOptions then Value:=1 else Value:=0;
...
function TLlPreviewControl26.PWideToString(pw: PWideChar): AnsiString;
var
  buffer : PAnsiChar;
  iLen : integer;
begin
...
procedure TLlPreviewControl26.QuestButtonState(ButtonID: integer;
...
procedure TLlPreviewControl26.RefreshOptions;
begin
...
procedure TLlPreviewControl26.RefreshToolbar;
begin
    if not FInitializing then
        SendMessage(handle, LS_VIEWERCONTROL_NTFY_TOOLBARUPDATE, 0, 0);
end;

procedure TLlPreviewControl26.SetBackgroundColor(const Value: TColor);
begin
...
procedure TLlPreviewControl26.SetCloseMode(const Value: TCloseMode);
begin
  FCloseMode := Value;
end;

procedure TLlPreviewControl26.SetFileName(const Value: TString);
begin
  if not (csDesigning in ComponentState) then
  begin
    if not FInitializing then
...
function TLlPreviewControl26.GetSaveAsFilePath: TString;
var Buffer: PTChar;
begin
...
procedure TLlPreviewControl26.SetShowToolbar(const Value: boolean);
...
procedure TLlPreviewControl26.SetShowUnprintableArea(const Value: boolean);
var IntValue: integer;
begin
...
procedure TLlPreviewControl26.SetToolbarButtons(
  const Value: TLlPreviewButtons);
begin
  FToolbarButtons := Value;
end;

procedure TLlPreviewControl26.SetZoom(Percentage: integer);
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_ZOOM, Percentage, 0);
end;


procedure TLlPreviewControl26.ZoomReset;
begin
    SendMessage(handle,LS_VIEWERCONTROL_RESET_ZOOM, 0, 0);
end;

procedure TLlPreviewControl26.ZoomRevert;
begin
    SendMessage(handle,LS_VIEWERCONTROL_POP_ZOOM, 0, 0);
end;

procedure TLlPreviewControl26.SelectSlideshowTransition;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SELECT_THEATERXFORM , 0, 0);
end;

procedure TLlPreviewControl26.ZoomTimes2;
begin
    SendMessage(handle,LS_VIEWERCONTROL_SET_ZOOM_TWICE, 0, 0);
end;

procedure TLlPreviewControl26.Attach(FParentComponent: TListLabel26;
  nFlags: Cardinal);
begin
...
procedure TLlPreviewControl26.SearchFirst(SearchString: TString; CaseSensitive: Boolean);
...
procedure TLlPreviewControl26.SearchNext;
begin
  SendMessage(handle, $400 + 29 (*LS_VIEWERCONTROL_SEARCH*), 1 (*LS_VCITEM_SEARCH_NEXT*), 0);
end;

function TLlPreviewControl26.CanClose: boolean;
begin
...
procedure TLlPreviewControl26.SetOnButtonClicked(
  const Value: TButtonClickedEvent);
begin
  FOnButtonClicked := Value;
end;

procedure TLlPreviewControl26.SetShowThumbnails(const Value: boolean);
var IntValue: integer;
begin
